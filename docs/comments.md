...
Generated by `npm run comments`. 6,843 comments, 14,433 lines of them, across 457 of 480 source files, in folder order.
...
> Low-contrast chrome keeps authored content visually prominent.


**L12**

> Pinned group headers must match the group background while scrolling.
...
> Field diagnostics replace hints to keep one message below each input.


**L73**

> Emphasize values that no longer inherit from their prefab.


**L111**

> Inline `top` offsets stack nested headers. Ancestor overflow disables sticky positioning.


**L119**

> Optional-section controls must shrink inside deeply nested groups.


**L132**

> Above a pinned header, so remove stays clickable where the two overlap.
...
> ── chips (balance & timeline views) ──────────────────────


**L175**

> ── problems: location, message, rationale, rule ───────────


**L185**

> Informational notes use lower contrast than actionable diagnostics.


**L197**

> Rule codes remain secondary to diagnostic messages.
...
> Array reorder controls stay compact because most lists are unordered.
...
> Show the prefab definition beside its affected instances.


**L341**

> Preserved overrides are expected fan-out results, not diagnostics.
...
> Studio shell. Everything here is scoped by CSS Modules so the classic
>   editor's stylesheet is never affected; the palette reuses the custom
>   properties globals.css defines on :root, so embedded schema forms and
>   analysis views match the shell without restyling.
...
> Slot 0 of the pinned-header stack: this names the top-level field, and the
>   nested .group-head.pinned headers offset themselves below it. Opaque, since
...
> Mods panel. A mod describes its UI and the host draws it, so these are the
>   only styles a mod's widget tree can reach — deliberately: an editor mod
>   cannot ship CSS, which is what keeps one mod from restyling the studio.
...
> The half the rename cannot do for you. Deliberately not styled as an error:
>   nothing is wrong yet, but somebody has to go and look.
...
> Only present while something is checked, so it never costs space it is not
>   using — the table is what the viewport is for.
...
> Whose it is. An unattributed field reads as part of the format, and would be
>   looked for in the schema by whoever inherits the module.
...
> The value a person put there, not the prefab's — shown so "reset" is a
>   decision rather than a leap.


**L692–693**

> A draft from a session that ended badly. Loud, because ignoring it loses it,
>   and it sits above everything until the author decides.


**L712**

> Form on the left, what it makes on the right — the preview is the point.
...
> ── The welcome shelf ──────────────────────────────────────────────────────
>   Shown before a world is open, which is a state the studio never used to have:
>   there was always a module on disk for the server to pick. A fresh browser has
>   an empty library, and this is the honest answer to that.
...
> Dark and dense, matching the studio. A reference page is read for a long
>   time in one sitting, so the chrome stays quiet and the tables carry the
>   contrast.
...
> The two applications, at the top of the sidebar on every page. Filled rather than outlined,
>   and full width rather than paired side by side: everything else in this column is a link to
>   prose about the software, and these two are the software. They should not read as the first
>   two entries of a list of documentation pages.
...
> The title and the dotted path sit on one line where there is room and stack
>   where there is not. Without the wrap a path like
>   `rules.spellcasting.componentActionTypes` is a flex item that will not shrink
>   below its own width, and it pushes the whole page sideways on a phone.
...
> Wide enough that a one line gloss stays close to one line. On a desktop the
>   column is already wider than this and the floor does nothing; on a phone it
>   is what stops every row growing to four lines of wrapped text.


**L147–151**

> The type column carries everything from `number` to a twelve value enum.
>   Under automatic table layout a `max-width` on the cell is ignored, so the
>   bound goes on an inline-block inside it, which the column does respect.
>   Below that width types wrap; above it nothing can force a horizontal
>   scrollbar on the whole table.
...
> Enum values as chips. They wrap as a group and read better than a run of
>   pipes once there are more than three or four of them.


**L163–165**

> Plain `inline`, not `inline-flex`: a flex container is block level enough
>   that a following `[]` cannot sit beside the last chip and drops to a line of
>   its own. Inline chips wrap the same way and keep the `[]` attached.
...
>   Not a plain run of inline <code>: JSX emits no whitespace between mapped
>   elements, so there is no break opportunity and the row runs off the page
>   however narrow its container is. Flex supplies both the wrap and the gap.


**L216–217**

> A single chip may be longer than a narrow screen, so let one break inside
>   itself as a last resort. Short chips never reach this.
...
> Prose tables carry paths and keys that must not be broken mid token, so
>   rather than wrap them the table scrolls inside itself, the same affordance
>   the generated field tables get. `display: block` is what makes the overflow
>   apply; the rows still lay out as a table through anonymous boxes.
