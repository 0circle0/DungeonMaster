/**
 * The claim this whole change exists to make: editing one thing writes one file.
 *
 * So these assert *counts*, not contents. A diff that returned the right bytes
 * for every file in the world would pass a contents test and be exactly the
 * design being replaced — the old save re-serialized 1.6 MB to change an integer
 * from 5 to 3, and it was correct the entire time.
 */
...
describe('diffProject', () => {
  ...
  it('writes only the manifest when a collection is reordered', () => {
    ...
    // The same entry objects in a different order — every name still maps to the
    // object it named, so no entry file has anything new to say.
  ...
  it('rewrites every entry a changed prefab is responsible for', () => {
    ...
    // The prefab changes and the document does not. The entry file is a recipe,
    // so it now expands to something other than what is on screen — it has to be
    // rewritten even though nobody touched it.
    ...
    // What it writes is whatever reproduces the entry on screen — here a recipe
    // that overrides the `kind` the moved template would otherwise supply. The
    // invariant is not "a recipe" or "a literal", it is that reading the files
    // back gives the author the world they were looking at.
  ...
  it('rewrites everything when identity is lost, with no special case for it', () => {
    const { snapshot } = stored();
    // What the raw-JSON editor does: parse a fresh copy. Nothing matches by
    // reference, so the ordinary algorithm reports the whole world.
   ...
   *
   * A file's text is decided by the entry, the prefab behind it, and the link
   * between them. The first two were watched and the third was not, so linking
   * an entry to a prefab that had not itself changed moved nothing the diff
   * looked at: it wrote no file, and the link was gone on the next open. It only
   * ever appeared to work because linking usually follows creating the prefab,
   * and a new prefab is dirty.
   */
  describe('a link is a reason to rewrite an entry', () => {
    ...
    it('writes the entry when it is linked to a prefab that did not change', () => {
      ...
      // The prefab is already stored and untouched. Only the second entry's
      // link appears.
      const after = linked({
    ...
    it('writes nothing when the link map is rebuilt with the same values', () => {
      ...
      // `recomputeInstances` returns a brand-new map with brand-new link objects
      // on every save, so this is the ordinary case and not an edge one. Compared
      // by reference, every linked entry in the world would be rewritten every
      // time.
  ...
  /**
   * `literal` was maintained on every save and read by nothing: the sidecar it
   * describes was never emitted, so a link whose entry could not be written as a
   * recipe was lost on the next open.
   */
  it('carries a link its entry file cannot express', () => {
    const poi = { id: 'ford', name: 'The Ford', kind: 'landmark' };
    const doc = { ...MINIMAL, world: { ...(MINIMAL['world'] as object), pointsOfInterest: [poi] } };
    // The template cannot produce `kind: landmark` from these params, and an
    // override cannot remove a key, so the file has to be literal.
    const prefab: Prefab = {
      id: 'inn',
      collection: 'world.pointsOfInterest',
    ...
    const { first } = stored(doc, authoring);
    // The point of the test: the entry could not be a recipe, so the sidecar is
    // the only place its link exists.
