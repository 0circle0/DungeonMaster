...
Vashta Qal is the module's second city. Two districts, fifty-five points of
interest, a spring, a glass market, a lens maker, a sunken bath behind a door
that already had a lock on it — and, until now, not one person in either
district and not one line of anything about any of it.

The other four empty areas are a salt town, a dry cistern with a village around
it, a ruined town coming out of a dune, and a field of fulgurite. Five dungeons
between them, none named by anything.

The four threads are all the same question asked four ways: **the water left,
and the leaving was not weather.**

  * **The Ninth Course** — the salt pans are cut in eight courses and the
    weighing house counts nine.
  * **What the Cistern Was For** — a cistern the size of a market, dry since
    before the village, and the village is named after it.
  * **The Bath That Is Still Full** — every well in Vashta Qal is metered and
    the bath under the Glass Quarter has never once been metered.
  * **The Town That Came Back** — the dune moved off a town in one season, and
    the town is the wrong age for the dune.

The Crater Stair is the over-tuned one: level 7 ground at the bottom of a level
...
GATES = [
    ...
    # The over-tuned one: level 7 under a level 6 district, and the seal names
    # both the lens and something that turns heat.
...
DIALOGUES.append(
    talk("glass_taj_talk", "greet",
         ...
         [rumour("glass_taj_metered",
                 ...
                 skill="insight"),
          # A Saltrun matter, told in Vashta Qal on purpose: every teller of
          # the courses thread lived in one town, and one missed conversation
          # would have ended it. `check_quests.py` counts the areas.
