...
Sixty-one of the continent's 108 areas are touched by no quest, no dialogue, no
NPC's home and no trigger. They hold 307 points of interest, among them 32
dungeon mouths, 20 ruins, 13 camps, 12 shrines and 159 shops and houses — all
...
moment a thread puts somebody in Whalebone Landing, the Landing stops being
empty and the rule would start rejecting the thread that filled it. So this is a
snapshot from before any of it, regenerated only when the *questlines* move:

    python3 -c "$(sed -n '/^REGENERATE/,/^\"\"\"/p' hiddenspace.py)"

Levels are the band each region's empty areas actually sit in, which is why a
thread never has to borrow ground from a questline to stay level-appropriate.
"""
