 ...
 *
 * Every sentence the engine produces lives in `narrative.systemText` and is
 * reached through here. There is deliberately **no English fallback**: a key
 * that resolves to nothing throws, because a silent empty string is exactly the
 * failure this whole mechanism exists to prevent. Nothing can reach that throw
 * through a normal load — `compileModule` proves every fragment is present and
 * every message keeps its placeholders before a game ever starts.
 *
 * A message may be a plain string or `{ pool }`, which hands it to
 * `narrative.textGrammar` and gets weighted, condition-gated variation for the
 * price of one field.
 */
 ...
 *
 * Events hold one of these rather than finished prose, so what the player reads
 * is decided by the module at render time and the same event can be rendered
 * into a terminal, a browser, or a different language.
 *
 * The `text` form is for prose the *module* already wrote — a dialogue option's
 * `lockedHint`, say. That is authored content passing through, not the engine
 * inventing a sentence, so it needs no key.
 */
export type Message =
...
function valueOf(module: CompiledModule, key: SystemTextKey): SystemTextValue {
  ...
  // Only reachable if a module was assembled without compiling it, which the
  // loaders make impossible. Naming the key beats rendering a blank.
  throw new Error(
 ...
 * Render one message.
 *
 * `sceneKey` gives a pool-backed message a stable identity, so the same refusal
...
/**
 * Several messages as one readable phrase.
 *
 * Used where a refusal has to name a list inside a sentence — the three things
 * a shop wants before it will deal with you, say.
 */
 ...
 *
 * Built once per module: the narrator runs several times a turn and these never
 * change, so re-resolving nine keys on every list would be pure waste.
 */
