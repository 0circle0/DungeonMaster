 ...
 *
 * A generated map answers "what might be here"; a static map answers "this is
 * here". Castles, shops, and set-piece rooms are authored as a grid of ids —
 * one id per cell, one grid per layer — and the engine uses them verbatim, no
 * rng consulted.
 *
 * On disk a static map is a folder (`maps/<id>/` beside `module.json`): a
 * `map.json` manifest plus one CSV per layer, assembled into this shape at
 * load. In the assembled document the grids are plain 2-D arrays, which is
 * what makes the ordinary machinery work unchanged: `collectRefs` proves every
 * cell resolves, `hashModule` hashes the content, and `extends` merges maps by
 * id like any other collection entry.
 *
 * Layers are ordered by draw order. The *kind* of a layer decides which
 * collection its cells resolve against — the same id could name a terrain and
 * an item, and the layer kind is what keeps that unambiguous.
 */
...
/**
 * Which collection each layer kind's cells resolve against. `markers` is null:
 * marker ids are free names ("entry", "door", "spawn", anything the author
 * wants findable), not references.
 */
...
/**
 * A plain union rather than a discriminated one, deliberately: the compiler's
 * `collectRefs` walks `ZodUnion` by descending the first branch that parses,
 * which is what turns every non-empty cell into a checked reference. Each
 * branch's `kind` literal makes the match unambiguous.
 */
...
export const staticMapSchema = z
  .object({
    ...
    tags,
    /**
     * The marker id the party arrives at. Required (via lint) when an area,
     * POI, or dungeon uses this map directly; a map only ever stamped into a
     * generated dungeon as a room needs `door` markers instead.
     */
  ...
  .strict()
  .superRefine((map, ctx) => {
    // Every layer must be the same rectangle — a sparse layer is authored as
    // empty cells, never as missing rows.
    ...
    // The base terrain layer makes the tiles total; without one there is no
    // ground to stand on.
