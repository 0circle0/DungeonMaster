 ...
 * The QuickJS host.
 *
 * One `QuickJSContext` per mod, not one shared: mods cannot see or pollute each
 * other's globals, and disposing a quarantined mod is a context disposal rather
 * than a cleanup problem.
 *
 * ## Why this variant
 *
 * `@jitl/quickjs-wasmfile-release-sync` ships the WASM as a real asset located
 * via `new URL(..., import.meta.url)`, which webpack emits as a static file.
 * The `singlefile` variants embed the WASM as a JS string containing raw binary
 * with octal escapes — Next's SWC minifier rewrites that literal into a
 * template string, where octal escapes are a **syntax error**. The build
 * succeeds and the browser then fails to load the chunk. Do not switch back
 * without loading a page in a real browser; `next build` passing proves
 * nothing here.
 *
 ...
 *
 * This is the only async step. Everything after it is synchronous, which is
 * what lets `reduce()` stay a synchronous pure function.
 */
...
export function createHost(options: HostOptions): SandboxHost {
  ...
  function install(mod: LoadedMod): InstallResult {
    ...
    // The entry file, then any other file the mod ships, are evaluated as one
    // script each. There is no module loader: a mod is a script, and `lib/*.js`
    // files are evaluated before the entry so the entry can use what they put
    // on `globalThis`.
    ...
    // Declared-but-unregistered is a warning: the manifest promised a hook the
    // code never installed, which is almost always a typo in one of the two.
    ...
    // Registered-but-undeclared is an error: the runtime indexes declarations
    // to decide whether to cross the boundary at all, so an undeclared handler
    // would simply never run, silently.
  ...
  function call(request: SandboxCall): SandboxResult {
    ...
    try {
      ...
      // A host-side throw (a disposed handle, an FFI fault) must not escape
      // into the engine either.
...
/**
 * QuickJS reports an interrupt and an out-of-memory as ordinary errors, so the
 * kind has to be read back off the message. Worth doing: "your mod is too slow"
 * and "your mod has a bug" need different fixes.
 */
